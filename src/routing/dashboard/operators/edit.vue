...
<v-container fluid grid-list-md>
     <v-layout row wrap justify-center align-center>
            <v-flex xs12 md6>
              <h1 class="text-xs-center title"> Update operator</h1>
              <v-form ref="form">
...
export default {
  ...
  computed: {
    operator() {
      return this.operators.find(el => el.id == this.id);
    },
    ...mapState({
  ...
  methods: {
    init() {
      ...
      (this.active = this.operator.active == 1 ? true : false),
        (this.streamIp = this.operator.streamServerIP);
      this.streamIp2 = this.operator.streamServerIP2;
      this.timeshift = this.operator.timeshiftIP;
    ...
    submit() {
      if (this.$refs.form.validate()) {
        ...
        this.$store.dispatch("updateOperator", body).then(() => {
          this.name = null;
          this.$router.go(-1);
        });
      }
